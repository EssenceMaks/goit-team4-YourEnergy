class c{constructor(){this.tasks=[],this.currentTaskId=null,this.modalEscapeListener=null,this.loadTheme(),this.loadTasks().then(()=>{this.init()})}async loadTasks(){try{const e=await(await fetch("../data/task-manager.json")).json();this.tasks=e.tasks||[],this.renderTasks()}catch(t){console.error("Error loading tasks:",t),this.tasks=[]}}async saveTasks(){try{const t={tasks:this.tasks};if(!(await fetch("/api/save-tasks",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})).ok)throw new Error("Failed to save tasks");console.log("Tasks saved successfully")}catch(t){console.error("Error saving tasks:",t)}}init(){this.setupEventListeners(),this.renderTasks(),this.setupDragAndDrop()}setupEventListeners(){console.log("Setting up event listeners");const t=document.getElementById("addTaskBtn");console.log("Add button:",t),t.addEventListener("click",()=>{console.log("Add button clicked"),this.currentTaskId=null,this.showModal()}),document.querySelector(".close").addEventListener("click",()=>{this.hideModal()}),document.getElementById("taskForm").addEventListener("submit",s=>{s.preventDefault(),this.handleFormSubmit()}),document.getElementById("themeToggleBtn").addEventListener("click",()=>{this.toggleTheme()}),document.getElementById("taskNumber").addEventListener("input",s=>{this.handleTaskNumberInput(s.target.value)}),document.addEventListener("click",s=>{s.target.closest(".task-number-container")||(document.getElementById("taskNumberSuggestions").style.display="none")}),document.getElementById("addAssigneeBtn").addEventListener("click",()=>{this.addAssigneeField()}),document.getElementById("assigneesList").addEventListener("click",s=>{if(s.target.classList.contains("remove-assignee-btn")){const n=s.target.closest(".assignee-item");n&&document.querySelectorAll(".assignee-item").length>1&&n.remove()}}),document.getElementById("addSubtaskBtn").addEventListener("click",()=>{this.addSubtaskField()}),document.getElementById("subtasksList").addEventListener("click",s=>{s.target.classList.contains("remove-subtask-btn")&&s.target.closest(".subtask-item").remove()})}addAssigneeField(){document.getElementById("assigneesList").appendChild(this.createAssigneeItem("empty"))}addSubtaskField(t="",e=!1){const s=document.getElementById("subtasksList"),n=document.createElement("div");n.className="subtask-item",n.innerHTML=`
        ...
        `,s.appendChild(n)}handleTaskNumberInput(t){const e=document.getElementById("taskNumberSuggestions"),s=document.getElementById("taskNumber");if(!t){s.classList.remove("exists","available"),e.style.display="none";return}const n=this.tasks.map(i=>i.taskNumber).filter(i=>i).sort((i,o)=>i-o),a=n.includes(t);if(s.classList.toggle("exists",a),s.classList.toggle("available",!a&&t.length>0),t.length>0){const i=n.filter(o=>o.toString().includes(t)).slice(0,5);i.length>0?(e.innerHTML=i.map(o=>`
                        <div class="suggestion-item" onclick="window.taskManager.selectTaskNumber('${o}')">
                            ${o}
                        </div>
                    `).join(""),e.style.display="block"):e.style.display="none"}else e.style.display="none"}selectTaskNumber(t){document.getElementById("taskNumber").value=t,document.getElementById("taskNumberSuggestions").style.display="none"}showModal(t=null){var o;const e=document.getElementById("taskModal"),s=document.getElementById("taskForm"),n=document.getElementById("assigneesList"),a=e.querySelector(".delete-task-btn");a.style.display=t?"block":"none",n.innerHTML="";const i=document.getElementById("subtasksList");if(i.innerHTML="",t){const r=this.tasks.find(d=>d.id===t);r&&(s.taskNumber.value=r.taskNumber||"",s.taskTitle.value=r.title,s.taskDescription.value=r.description,s.taskCategory.value=r.category,s.taskPriority.value=r.priorityStatus,s.taskStatus.value=r.progressStatus,(r.assignees||[r.assignee||"empty"]).forEach(l=>{n.appendChild(this.createAssigneeItem(l))}),(o=r.subtasks)==null||o.forEach(l=>{this.addSubtaskField(l.text,l.completed)}),this.currentTaskId=t)}else s.reset(),s.taskCategory.value="Must_Have",n.appendChild(this.createAssigneeItem("empty"));e.style.display="block",a.onclick=()=>this.showDeleteConfirmation(t),this.modalEscapeListener=r=>{r.key==="Escape"&&this.hideModal()},document.addEventListener("keydown",this.modalEscapeListener),e.addEventListener("click",r=>{r.target===e&&this.hideModal()})}showDeleteConfirmation(t){const e=document.getElementById("deleteConfirmModal");e.style.display="block";const s=()=>{this.deleteTask(t),e.style.display="none",this.hideModal(),o()},n=()=>{e.style.display="none",o()},a=r=>{r.target===e&&(e.style.display="none",o())},i=r=>{r.key==="Escape"&&(e.style.display="none",o())},o=()=>{document.getElementById("confirmDelete").removeEventListener("click",s),document.getElementById("cancelDelete").removeEventListener("click",n),e.removeEventListener("click",a),document.removeEventListener("keydown",i)};document.getElementById("confirmDelete").addEventListener("click",s),document.getElementById("cancelDelete").addEventListener("click",n),e.addEventListener("click",a),document.addEventListener("keydown",i)}hideModal(){const t=document.getElementById("taskModal");t.style.display="none",this.modalEscapeListener&&(document.removeEventListener("keydown",this.modalEscapeListener),this.modalEscapeListener=null)}handleFormSubmit(){const t=document.getElementById("taskForm"),e=Array.from(t.querySelectorAll(".subtask-item")).map(a=>({id:Date.now()+Math.random().toString(36).substr(2,9),text:a.querySelector(".subtask-text").value,completed:a.querySelector(".subtask-checkbox").checked})),s=Array.from(t.querySelectorAll(".taskAssignee")).map(a=>a.value).filter(a=>a);s.length===0&&s.push("empty");const n={title:t.taskTitle.value,taskNumber:t.taskNumber.value||null,description:t.taskDescription.value,category:t.taskCategory.value,priorityStatus:t.taskPriority.value,progressStatus:t.taskStatus.value,assignees:s,subtasks:e};if(n.taskNumber&&this.tasks.some(i=>i.taskNumber===n.taskNumber&&i.id!==this.currentTaskId)){alert("Задача с таким номером уже существует!");return}this.currentTaskId?this.updateTask(this.currentTaskId,n):this.addTask(n),this.hideModal(),this.renderTasks()}addTask(t){const e={id:Date.now().toString(),taskNumber:t.taskNumber||null,title:t.title,description:t.description,category:t.category||"Must_Have",priorityStatus:t.priorityStatus,progressStatus:t.progressStatus,assignees:t.assignees,subtasks:t.subtasks||[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};console.log("New task:",e),this.tasks.push(e),this.saveTasks()}updateTask(t,e){const s=this.tasks.findIndex(n=>n.id===t);s!==-1&&(this.tasks[s]={...this.tasks[s],taskNumber:e.taskNumber,title:e.title,description:e.description,category:e.category,priorityStatus:e.priorityStatus,progressStatus:e.progressStatus,assignees:e.assignees,subtasks:e.subtasks,updatedAt:new Date().toISOString()},this.saveTasks())}deleteTask(t){this.tasks=this.tasks.filter(e=>e.id!==t),this.saveTasks(),this.renderTasks()}renderTasks(){document.querySelectorAll(".drop-zone").forEach(s=>{s.innerHTML=""}),this.tasks.forEach(s=>{if(!s.assignees&&s.assignee&&(s.assignees=[s.assignee],delete s.assignee),s.assignees.some(a=>a!=="empty")){const a=this.createTaskElement(s),i=document.querySelector(`.drop-zone[data-priority="${s.priorityStatus}"][data-status="${s.progressStatus}"]`);i&&i.appendChild(a)}});const t=document.querySelector(".must-have-grid");if(t){t.innerHTML="";const s=this.tasks.filter(a=>a.category==="Must_Have"),n=this.groupTasksByAssignee(s,!0);this.renderGroupedTasks(n,t)}const e=document.querySelector(".upgrade-grid");if(e){e.innerHTML="";const s=this.tasks.filter(a=>a.category==="Upgrade"),n=this.groupTasksByAssignee(s,!0);this.renderGroupedTasks(n,e)}}createTaskElement(t){var n;const e=document.createElement("div");e.className="task-card",e.draggable=!0,e.dataset.taskId=t.id;const s=(t.assignees||[t.assignee||"empty"]).filter(a=>a!=="empty");if(e.innerHTML=`
        ...
        `,((n=t.subtasks)==null?void 0:n.length)>0){const a=t.subtasks.filter(o=>o.completed).length,i=t.subtasks.length;e.querySelector(".task-left").insertAdjacentHTML("beforeend",`
            ...
            `)}return e.addEventListener("click",()=>this.showModal(t.id)),e}setupDragAndDrop(){console.log("Setting up drag and drop"),document.addEventListener("dragstart",e=>{console.log("Drag started:",e.target),e.target.classList.contains("task-card")&&(e.target.classList.add("dragging"),e.dataTransfer.setData("text/plain",e.target.dataset.taskId))}),document.addEventListener("dragend",e=>{e.target.classList.contains("task-card")&&e.target.classList.remove("dragging")}),document.querySelectorAll(".drop-zone").forEach(e=>{e.addEventListener("dragover",s=>{s.preventDefault(),e.classList.add("dragover")}),e.addEventListener("dragleave",s=>{e.classList.remove("dragover")}),e.addEventListener("drop",s=>{s.preventDefault(),e.classList.remove("dragover");const n=s.dataTransfer.getData("text/plain"),a=this.tasks.find(i=>i.id===n);a&&(!a.assignees&&a.assignee&&(a.assignees=[a.assignee],delete a.assignee),a.priorityStatus=e.dataset.priority,a.progressStatus=e.dataset.status,a.updatedAt=new Date().toISOString(),this.saveTasks(),this.renderTasks())})})}loadTheme(){const t=localStorage.getItem("theme")||"dark";document.documentElement.setAttribute("data-theme",t)}toggleTheme(){const e=document.documentElement.getAttribute("data-theme")==="dark"?"light":"dark";document.documentElement.setAttribute("data-theme",e),localStorage.setItem("theme",e)}groupTasksByAssignee(t,e=!1){const s={};return t.forEach(n=>{(n.assignees||[n.assignee||"empty"]).forEach(i=>{(i!=="empty"||e)&&(s[i]||(s[i]=[]),s[i].includes(n)||s[i].push(n))})}),s}renderGroupedTasks(t,e){Object.entries(t).forEach(([s,n])=>{const a=document.createElement("div");a.className="task-assignee",a.innerHTML=`
                ...
                `).join("")}
            `,a.addEventListener("click",o=>{const r=o.target.dataset.taskId;r&&this.showModal(r)}),e.appendChild(a);const i=document.createElement("div");i.className="task-content",i.innerHTML=n.map(o=>`
            ...
            `).join(""),i.addEventListener("click",o=>{const r=o.target.closest(".task-item");if(r){const d=r.dataset.taskId;this.showModal(d)}}),e.appendChild(i)})}createAssigneeSelect(){return`
...
//# sourceMappingURL=task-manager-CjRrTwQv.js.map
